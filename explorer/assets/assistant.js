...
function toggleChat() {
  ...
  if (isOpen) {
    // Auto-initialize engine when chat opens
    if (!chatState.engine) {
      initEngine();
    }
    els.input.focus();
...
async function initEngine() {
  ...
  const FASTEST_MODEL = "Llama-3.2-1B-Instruct-q4f16_1-MLC";
  
  if (chatState.engine && chatState.currentModel === FASTEST_MODEL) {
  ...
  try {
    ...
    chatState.currentModel = FASTEST_MODEL;

    els.loadingText.textContent = "Loading Llama 3.2 1B (fastest)...";

    // Create engine with progress callback
    chatState.engine = await chatState.webllm.CreateMLCEngine(FASTEST_MODEL, {
...
// Model selection removed - always uses fastest model automatically
...
async function handleSubmit(e) {
  ...
  if (!userMessage || chatState.isGenerating) return;
  
  // Auto-initialize engine if needed
  if (!chatState.engine) {
    await initEngine();
    if (!chatState.engine) {
      addMessage("assistant", "Please wait for the model to finish loading...");
      return;
    }
  }
